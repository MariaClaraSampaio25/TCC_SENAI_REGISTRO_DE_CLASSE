    ...
    <header class="cabecalho">
      ...
      <div class="col">
        ...
        <div class="card-header py-3">
            <center> <h2 class="my-0 fw-normal"><b><svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" class="bi bi-diagram-3" viewBox="0 0 16 16">
...
</svg>&nbsp;&nbsp;Matricular Alunos</b></h2></center>
          </div>
          <div class="card-body" style="text-align: left;">
          <form name="cad_aluno_turma" action="cad_aluno_turma.php" method="POST">
            <label class='form-label'>Selecione uma Turma</label><br/>
            <select name="Id_turma" class="form-control">
                    <option hidden selected>Clique aqui para selecionar uma opção</option>
                    <?php
                      include 'conecta.php';
                      $pesquisa = mysqli_query($conn, "SELECT * FROM turma");
                      $row = mysqli_num_rows($pesquisa);
                      if($row > 0){
                        while($registro2 = $pesquisa-> fetch_array()){
                          $Id = $registro2['Id'];
                          $Nome = $registro2['Nome'];
                        ...
                        echo '<option value="">Não há turmas formadas!</option>';
                      }
                    ?>
                </select>
            ...
            <tr>
                ...
                <th class="letra" scope="col">Nome do aluno</th>            
            </tr>
            </thead>
            <tbody>
            <?php
                include 'conecta.php';
                $pesquisa = mysqli_query($conn, "SELECT Id, Nome FROM aluno WHERE Id NOT IN (SELECT Id_aluno FROM aluno_turma)");
                $row = mysqli_num_rows($pesquisa);
                if($row > 0){
                    while($registro = $pesquisa-> fetch_array()){
                        ...
                        echo '<td class="letra">'.$registro['Nome'].'</td>';
                        echo '</tr>';
                    }
                    echo '</tbody>';
                    echo '</table>';
                    echo '<br/><br/>';
                    echo '<input type="submit" class="w-100 btn btn-lg btn btn-outline-success" value="Matricular aluno"/><br/><br/>';
                }
                else {
                    echo '</tbody>';
                    echo '</table>';
                    echo "<tr><td colspan='1'><b>Todos os alunos estão matriculados em alguma das turmas!</b></td></tr>";
                }
            ?>
            <br/><br/>
          </form>
